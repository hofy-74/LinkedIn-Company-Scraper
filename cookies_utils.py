...
def load_cookie_file(path: str):
    ...
    try:
        ...
        raise CookieError(
            ...
            "Make sure it's in the project folder, or set LINKEDIN_COOKIES_PATH to point at it."
        )
    except (pickle.UnpicklingError, EOFError) as e:
        raise CookieError(f"The cookie file is corrupted or not a valid pickle: {e}")

    if not isinstance(cookies, list) or not cookies:
        raise CookieError("The cookie file is empty or not in the expected format (should be a list of dicts).")

    names = {c.get("name") for c in cookies if isinstance(c, dict)}
    missing = REQUIRED_COOKIE_NAMES - names
    if missing:
        raise CookieError(
            "The cookies are missing a required session cookie ({}). "
            "The session has likely expired - log in again with a separate script "
            "and export the cookies fresh."
            .format(", ".join(missing))
        )
    return cookies
