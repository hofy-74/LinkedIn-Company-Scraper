...
st.markdown(
    ...
    <div class="app-subtitle">
        ...
        profile — industry, size, HQ, founding year, and a few other fields —
        straight into an Excel file you can keep adding to.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    if cookies_ready:
        st.markdown(
            '<span class="status-pill pill-ok">● Logged in</span>',
            unsafe_allow_html=True,
        )
        st.caption(f"Reading session from `{COOKIES_PATH}`")
    else:
        st.markdown(
            '<span class="status-pill pill-bad">● Not connected</span>',
            unsafe_allow_html=True,
        )
        st.caption(
            f"I couldn't find `{COOKIES_PATH}`. Drop it next to `app.py`, or point "
            "`LINKEDIN_COOKIES_PATH` at wherever it lives."
        )

    st.markdown("---")
    st.subheader("How this works")
    st.caption(
        "It signs in with a saved LinkedIn session (cookies) instead of a "
        "username and password, and runs the browser headless in the "
        "background. Between steps it waits a random couple of seconds "
        "rather than firing requests back-to-back."
    )

    st.markdown("---")
    st.caption(
        "⚠️ A quick heads-up: scraping LinkedIn like this isn't something "
        "they allow in their Terms of Service, and the account tied to "
        "these cookies could get flagged if you lean on this too hard. "
        "Built as a portfolio piece — use it lightly, not as a daily driver."
    )
