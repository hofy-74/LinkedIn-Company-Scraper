...
# Optional explicit paths to a Chrome/Chromium binary and its matching
# chromedriver. Leave unset to auto-detect (checks common binary names on
# PATH first). Setting these explicitly is the reliable option on Streamlit
# Community Cloud, where packages.txt installs Chromium + chromium-driver
# to a fixed system location rather than relying on Selenium's own
# runtime auto-download, which can fail in a minimal container.
